...
Source: https://dapp.org.uk/reports/uniswapv2.html
...
An overview of the formal specifications and their proof status can be found in the audit teams CI system at dapp.ci/k-uniswap, and a discussion on the nature of the proofs produced can be found in the Formal Verification section of this report.
...
Before there is any liquidity in the pool, the attacker sends a small amount of both tokens of the Pair, receiving a small amount of liquidity pool shares ("LP Tokens").
They then send a very large amount of the Pair tokens to the contract, and invokes sync.
...
Before uniswap-v2-core@d1c8612e, the initial value to the iteration was given as x = (y + 1) / 2, which results in an integer overflow when y = uint(-1), setting x to zero, leading to a division by zero in the calculation of x_(n+1).
...
A manual proof of correctness of this algorithm is given here.
...
Factory: use .creationCode when retrieving Pair bytecode
The createPair method of UniswapV2Factory stored the bytecode for a Pair as a raw bytes value set during construction. When deploying a new Pair, the bytecode would have to be loaded into memory from storage, word-by-word. The audit team recommended that this storage variable be removed, and replaced with the .creationCode attribute of the UniswapV2Pair contract, simplifying the code and clarifying intent.
...
2256⟺2256⟺ΔT=Pcumulative=2112⋅Reserve1Reserve0⋅ΔT=2144⋅(Reserve0Reserve1)
...
ΔT=2144⋅(Reserve0Reserve1)=2144⋅10−36≈2.23×107≈8.52months
...
Deduct the fee from the amount credited to the transfer recipient
Transfer the full amount and deduct the fee from the sender's balance
...
transfer requested output amounts to recipient
if the data argument is non-empty, call into recipient with user provided calldata
calculate inputs
check the fee adjusted constant product invariant
...
(x1−0.003⋅xin)⋅(y1−0.003⋅yin)≥x0⋅y0(1)
This ensures that:
...
Arithmetic Overflow and Numeric Range
...
There are two square root computations, used to calculate the change in the k
 invariant since the last call to _mintFee. In principle, this can result in liquidity providers being charged a slightly higher fee than what they should be charged "in theory", if the second sqrt calculation happens to have a larger error than the first. However, this is not economically significant since the error is small, and most importantly does not accumulate and is bounded in time.
...
Proof of Correctness of sqrt
Below we prove that sqrt will terminate within 255 loop iterations, when called with any input from 0
 to 2256−1
 (the actual maximum number of loop iterations appears to be 135), and that return value z is the largest integer such that z^2 <= y, i.e. that z is the square root of y rounded down to the nearest integer.

Some care must be taken when handling truncating division, which is the only difference from the standard proof: let 𝚍𝚒𝚟(𝚡,𝚢)
 denote the integer result of dividing x
 by y
 without remainder. Given an integer y
 greater than 3, define the sequence (zn)
 of positive integers by as follows:

z0z1=y=𝚍𝚒𝚟(y,2)+1
and for all other n
:

zn+1={𝚍𝚒𝚟(𝚍𝚒𝚟(y,zn)+zn,2)znif 𝚍𝚒𝚟(𝚍𝚒𝚟(y,zn)+zn,2)<znotherwise
It is obvious that this sequence stabilises at some zN
, since its tail is a sequence of monotonically decreasing positive integers. Furthermore, whenever zn>y√
, we have zn+1<zn
, since:

𝚍𝚒𝚟(𝚍𝚒𝚟(y,zn)+zn,2)≤12(𝚍𝚒𝚟(y,zn)+zn)≤yzn+zn2<z2nzn+zn2=zn
From this it follows that the limit must satisfy zN≤y√
.

It's clear enough that this sequence models precisely the computation in sqrt (which we have thus proved terminates), that the limit is the return value, and N
 the number of iterations that the loop runs for.

It is easy to see that N≤y−y√
. We can strengthen this to a more useful bound by defining the error sequence ϵn=zn−y√
, and proving that ϵn<2−nϵ1
 for n<N
 (the proof is essentially the same as in the infinite-precision setting). Thus, as ϵ1≤2255
, we can conclude that N<255
 for all inputs y<2256
.

It remains only to bound the final error y√−zN
. We do this by defining the sequence of integers rn
 such that y=zn⋅𝚍𝚒𝚟(y,zn)+rn
, so that 0≤rn<zn
 for each n
, and decomposing zN
 as a sum:

zN=12(y−rN−1zN−1+zN−1)=12(yzN−1+zN−1)−zN−1+rN−12zN−1
The first term is strictly greater than y√
, by the AM-GM inequality. The second term is strictly less than 1
, due to the fact that rN−1<zN−1
. This proves that ϵN<1
, or in other words, the result z is the largest integer such that z^2 <= y. ◼
...
They do not, per se, make claims about invariants that should hold across multiple calls (contract level invariants), such as the xy=k
 constant product invariant (or its fee-adjusted counterpart).
...
Compilation of the act specs into some high level proof language (e.g. Coq, Agda)
An inductive proof implemented using post-conditions in the generated K specs
Verification of external calls to unknown code
...
Verification of sqrt bytecode
The team believes that it should be possible to prove a loop invariant by coïnduction showing the EVM implementation of sqrt agrees with the model used in the manual proof of convergence.
